...
export async function POST(req: NextRequest) {
  try {
    ...
    const tipe_pajak = formData.get("tipe_pajak");
    const status = formData.get("status");
    const tanggungan = formData.get("tanggungan");
    const gaji_pokok = formData.get("gaji_pokok");
    const npwp = formData.get("npwp");
    const bpjs_health = formData.get("bpjs_health");
    const bpjs_employment = formData.get("bpjs_employment");
    ...
    qb.column("tgl_join");
    qb.column("jabatan");
    qb.column("tipe_pajak");
    qb.column("status");
    qb.column("tanggungan");
    qb.column("gaji_pokok");
    qb.column("npwp");
    qb.column("bpjs_health");
    qb.column("bpjs_employment");
    ...
    for (const dt of json_tunjangan_pegawai) {
      pool.execute(qbTP.insert(), [insertedId, dt.id_tunjangan, dt.nominal]);
    }
