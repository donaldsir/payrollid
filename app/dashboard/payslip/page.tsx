...
export default function Page() {
  ...
  const changeAmount = () => {
    setDetailBruto((prevItems: any) =>
      prevItems.map((item: any) => (item.nama_tunjangan === nama_tunjangan ? { ...item, nominal: edit_amount } : item))
    );
  ...
  const calculate = async () => {
    ...
    const res = await fetch(`/api/employee/${id_pegawai}/payslip`, { method: "GET" });
    const data = await res.json();
    ...
    <>
      ...
      <SimpleGrid column={1} mt={4} background="gray.50" p={4}>
        <Text fontSize="sm" textAlign="center" fontWeight="bold">
          Payslip - {getMonthName(bulan)?.label} {tahun}
        </Text>
      ...
      <Grid templateColumns="repeat(2, 1fr)" gap={4} mt={4}>
        <GridItem background="gray.50" p={4}>
          ...
          {detail_bruto.map((row: any, index) => (
            <Grid key={index} templateColumns="repeat(2, 1fr)" gap={10} p={1}>
              <GridItem>
                <Text fontSize="sm">{row.nama_tunjangan}</Text>
              </GridItem>
              <GridItem>
        ...
        <GridItem background="gray.50" p={4}>
          ...
          <Grid templateColumns="repeat(2, 1fr)" gap={10} p={1}>
            <GridItem>
              <Text fontSize="sm">Tax Income (PPh)</Text>
            </GridItem>
            <GridItem>
              <Text fontSize="sm" textAlign="right">
          ...
          <Grid templateColumns="repeat(2, 1fr)" gap={10} p={1}>
            ...
            <GridItem>
              <Text fontSize="sm" fontWeight="bold" textAlign="right">
                {detail_bruto.reduce((sum: number, row: any) => sum + row.nominal, 0).toLocaleString("id-ID")}
              </Text>
            </GridItem>
          </Grid>
        </GridItem>
      ...
      <Grid templateColumns="repeat(2, 1fr)" gap={10} p={3} mt={6} background="red.700" color="white">
        ...
        <GridItem>
          <Heading fontWeight="bold" size="md" textAlign="right">
            {(detail_bruto.reduce((sum: number, row: any) => sum + row.nominal, 0) - pph).toLocaleString("id-ID")}
          </Heading>
        </GridItem>
      </Grid>
