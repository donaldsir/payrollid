...
export default function Page() {
    ...
    const getData = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/etr/${id}`, { method: 'GET' });
            ...
            setNamaETR(data?.etr.nama_etr || "");
            setKeterangan(data?.etr.keterangan || "");
            setETRPTKP(data?.etr_ptkp || []);
            setETRBruto(data?.etr_bruto || []);
            setLoading(false);
        } catch (error) {
    ...
    const update = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const res = await fetch(`/api/etr/${id}`, {
                method: "POST",
                body: formData,
            });
